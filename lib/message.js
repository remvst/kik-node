...
class Message {
    ...
    toJSON() {
        ...
        if (state.suggestedResponses && state.suggestedResponses.length !== 0) {
            json.suggestedResponses = state.suggestedResponses;
        }
    ...
    addTextResponse(text) {
        let responses = this._state.suggestedResponses || [];

        for (let i = 0; i < arguments.length; ++i) {
            responses.push('' + arguments[i]);
        }

        this._state.suggestedResponses = responses;
